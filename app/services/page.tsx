          ...
          <>
                <section className="bg-navy py-16 px-4">
                        <div className="max-w-7xl mx-auto">
                                  <p className="section-label">Production Services</p>p>
                                  <h1 className="section-title-light mt-2 mb-4 max-w-3xl">Full-Service Event Production</h1>h1>
                ...
                <section className="bg-white py-16 px-4">
                        <div className="max-w-7xl mx-auto">
                          ...
                          <Link
                                          ...
                                          >
                                          <div className="text-4xl mb-4">{s.icon}</div>div>
                                          <h2 className="text-xl font-bold text-navy mb-2 group-hover:text-accent transition-colors">
                                            {s.title}
                                          </h2>h2>
                                          <p className="text-gray-600 text-sm leading-relaxed mb-4">{s.description}</p>p>
                                          <ul className="space-y-1 mb-4">
                                            {s.capabilities.slice(0, 3).map((c) => (
                                                                <li key={c} className="text-xs text-gray-500 flex gap-2">
                                                                                      <span className="text-accent flex-shrink-0">{'\u2713'}</span>span>
                                                                  {c}
                                                                </li>li>
                                                              ))}
                                          </ul>ul>
                                          <span className="text-accent text-sm font-semibold">Learn more &rarr;</span>span>
                          </Link>Link>
                        ))}
                                  </div>div>
                        </div>div>
            ...
            {/* CTA */}
                <section className="bg-navy py-14 px-4 text-center">
                        <div className="max-w-2xl mx-auto">
                                  <h2 className="section-title-light mb-4">Need Multiple Services?</h2>h2>
