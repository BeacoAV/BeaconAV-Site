export interface City {
  slug: string; name: string; state: string; stateAbbr: string; tier: 1 | 2 | 3;
  intro: string; venues: string[]; suburbs: string[]; eventNote: string;
}

export const cities: City[] = [
  // -- TIER 1 --
  { slug: 'austin-tx', name: 'Austin', state: 'Texas', stateAbbr: 'TX', tier: 1,
  ...
  { slug: 'new-york-ny', name: 'New York', state: 'New York', stateAbbr: 'NY', tier: 1,
  ...
  { slug: 'los-angeles-ca', name: 'Los Angeles', state: 'California', stateAbbr: 'CA', tier: 1,
    ...
    venues: ['Los Angeles Convention Center','Crypto.com Arena','Beverly Hilton','Dolby Theatre','SoFi Stadium','Petersen Automotive Museum','The Broad Stage'],
  ...
  { slug: 'chicago-il', name: 'Chicago', state: 'Illinois', stateAbbr: 'IL', tier: 1,
  ...
  { slug: 'miami-fl', name: 'Miami', state: 'Florida', stateAbbr: 'FL', tier: 1,
    ...
    venues: ['Miami Beach Convention Center','Biltmore Hotel','P--rez Art Museum Miami','JW Marriott Marquis Miami','Faena Forum','Wynwood Walls Event Space','Hard Rock Stadium'],
  ...
  { slug: 'dallas-tx', name: 'Dallas', state: 'Texas', stateAbbr: 'TX', tier: 1,
    intro: "Dallas is a major corporate event hub -- home to Fortune 500 headquarters and some of Texas's largest convention facilities. Our Dallas-Fort Worth production network handles events of every scale.",
  ...
  { slug: 'las-vegas-nv', name: 'Las Vegas', state: 'Nevada', stateAbbr: 'NV', tier: 1,
  ...
  // -- TIER 2 --
  { slug: 'houston-tx', name: 'Houston', state: 'Texas', stateAbbr: 'TX', tier: 2,
  ...
  { slug: 'phoenix-az', name: 'Phoenix', state: 'Arizona', stateAbbr: 'AZ', tier: 2,
  ...
  { slug: 'denver-co', name: 'Denver', state: 'Colorado', stateAbbr: 'CO', tier: 2,
  ...
  // -- TIER 3 --
  { slug: 'seattle-wa', name: 'Seattle', state: 'Washington', stateAbbr: 'WA', tier: 3,
    intro: "Seattle's tech-heavy economy -- Amazon, Microsoft, Boeing -- generates consistent demand for high-caliber corporate event production. Our Seattle network handles everything from intimate executive briefings to large convention productions.",
  ...
  { slug: 'atlanta-ga', name: 'Atlanta', state: 'Georgia', stateAbbr: 'GA', tier: 3,
  ...
  { slug: 'nashville-tn', name: 'Nashville', state: 'Tennessee', stateAbbr: 'TN', tier: 3,
  ...
  { slug: 'charlotte-nc', name: 'Charlotte', state: 'North Carolina', stateAbbr: 'NC', tier: 3,
  ...
  { slug: 'boston-ma', name: 'Boston', state: 'Massachusetts', stateAbbr: 'MA', tier: 3,
...
export function getCityBySlug(slug: string): City | undefined { return cities.find((c) => c.slug === slug); }
export function getAllCitySlugs(): string[] { return cities.map((c) => c.slug); }
export function getCitiesByTier(tier: 1 | 2 | 3): City[] { return cities.filter((c) => c.tier === tier); }
